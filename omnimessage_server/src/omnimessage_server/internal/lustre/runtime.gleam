// This is an extension of Lustre's runtime with support for `UpdateSubscribe`
// See original in:
// https://github.com/lustre-labs/lustre/blob/main/src/lustre/internals/runtime.gleam
...
type State(model, msg, runtime) {
  State(
    ...
    external_dispatches: Dict(String, fn(msg) -> Nil),
  )
}
...
pub type Action(msg, runtime) {
  ...
  Shutdown
  PatchSubscribe(String, fn(Patch(msg)) -> Nil)
  PatchUnsubscribe(String)
  UpdateSubscribe(String, fn(msg) -> Nil)
  DispatchUnsubscribe(String)
}
...
fn loop(
  ...
  case message {
    ...
    Batch([msg], other_effects) -> {
      ...
      run_external_dispatches(state.external_dispatches, msg)
    ...
    UpdateSubscribe(id, dispatch) -> {
      let external_dispatches =
        dict.insert(state.external_dispatches, id, dispatch)
      let next = State(..state, external_dispatches:)

      actor.continue(next)
    }

    DispatchUnsubscribe(id) -> {
      let external_dispatches = dict.delete(state.external_dispatches, id)
      let next = State(..state, external_dispatches:)
...
@target(erlang)
fn run_external_dispatches(
  external_dispatches: Dict(any, fn(msg) -> Nil),
  message: msg,
) -> Nil {
  use _, external_dispatch <- dict.each(external_dispatches)
  external_dispatch(message)
}
